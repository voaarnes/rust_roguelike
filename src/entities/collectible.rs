...
impl Plugin for CollectiblePlugin {
   fn build(&self, app: &mut App) {
       app
           ...
           .add_systems(OnEnter(GameState::InGame), (build_collectible_atlas, ApplyDeferred, seed_collectibles).chain().after(TilemapSet::LoadLevel))
           .add_systems(Update, handle_spawn_collectible_events.run_if(in_state(GameState::InGame)))
           .add_systems(Update, animate_collectibles.run_if(in_state(GameState::InGame)));
   }
}
